      ...
      <?php
                    ...
                    $con = mysqli_connect( 'localhost', 'gaeun', 'testtest', 'z_mosashop');
                          ...
                          $ordernumber = time().mt_rand(1, 10000); // 주문번호 (한번에 함께 주문한 상품들은 주문번호가 같음) 현재시간+1~10,000사이의 난수
                            ...
                            for( $iis = 0; $iis < $n ; $iis++ ) { // array에 담긴 값을 차례로 반복문으로 꺼내기 위해 $iis = 0으로 초기값을 세팅함

                                      $sproduct_id = $product_id[$iis];
                                      $sproduct_amount = $product_amount[$iis];
                                      $sproduct_price = $product_price[$iis];
                                  ...
                                  $querysave = "INSERT into orderorder (ordernumber, customer_id, product_id, product_amount, product_price, order_date, status,
                                                ...
                                                values ('$ordernumber', '$customer_id', '$sproduct_id', '$sproduct_amount', '$sproduct_price', '$order_date', '결제완료',
                                                 '$receivername', '$phonenumber', '$address_num', '$address_main', '$address_det', '$address_ref')";
                                  $result= mysqli_query($con, $querysave);

                                      $sbasket_no = $basket_no[$iis];

                                  $querydelete = "DELETE from basket where customer_id = '$customer_id' and product_id = '$sproduct_id'";
                                  $resultdel = mysqli_query($con, $querydelete);
                            }
                    ?>
