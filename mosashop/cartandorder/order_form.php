...
<?php
  ...
  $no = $_GET['no'];
  $customer_id = $_POST['customer_id'];  //  echo "customer id ";   echo $customer_id;
  $totalprice = $_POST['totalprice'];    //  echo "totalprice ";     echo $totalprice;
  $order_date = $_POST['order_date'];    //  echo "order_date ";   echo $order_date;
  $status = $_POST['status'];            //  echo "status ";   echo $status;

  $basketid = $_POST['checkb']; // 상품id 배열로 저장해둠
  $n = count($basketid); // 넘어온 체크 아이템 개수 (반복문 쓸 때 쓰기)

  $product_id = $_POST['product_id']; // 여러 개 상품id값이 넘어오므로 배열로 넘어옴 꺼낼때 반복문으로 꺼내야 함
  $product_amount = $_POST['product_amount']; // 여러 개 상품수량값이 넘어오므로 배열로 넘어옴 꺼낼때 반복문으로 꺼내야 함
  $product_price = $_POST['product_price2']; // 계산과정 생략하기 위해 (여러개 상품수량*상품가격 = 총가격 값)을 여러개 넘김. 배열로 넘어옴 꺼낼때 반복문으로 꺼내야 함

  $idarray = array($product_id); // 받아온 값을 array에 담음
  $amountarray = array($product_amount); // 받아온 값을 array에 담음
  $pricearray = array($product_price); // 받아온 값을 array에 담음

 $connne = mysqli_connect( 'localhost', 'gaeun', 'testtest', 'z_mosashop');
 $queryse = "SELECT * from member_info where no = $customer_id"; // 주소정보를 가져오기 위해 쿼리문 연결함
 $resulte = mysqli_query($connne, $queryse);
 $row_e = mysqli_fetch_array($resulte);                                                                       ?>
  ...
  /*    font-family: Lobster;  */
    ...
    $( function(){
        ...
        $('#address_ref').attr( "readonly", "true" );
                ...
                $('#receiver').val("<?php echo $row_e[name]; ?>");
                $('#phonenum').val("<?php echo $row_e[phone_number]; ?>");
                $('#address_num').val("<?php echo $row_e[address_number]; ?>");
                $('#address_main').val("<?php echo $row_e[address_main]; ?>");
                $('#address_detail').val("<?php echo $row_e[address_detail]; ?>");
                $('#address_ref').val("<?php echo $row_e[address_ref]; ?>");
...
<?php if(!isset($_SESSION['email']) || !isset($_SESSION['name'])){ // 세션 없을때
    ?> <a href="../login/login.php">로그인</a>
       ...
       <div style="width: 660px; padding-top: 50px; padding-bottom: 50px; margin: 0 auto; font-size: 30px;">
         장바구니&nbsp;&nbsp;&nbsp;&nbsp;>&nbsp;&nbsp;&nbsp;&nbsp;<span style="font-size: 33px; font-weight: bold;">주문서 작성</span>&nbsp;&nbsp;&nbsp;&nbsp;>&nbsp;&nbsp;&nbsp;&nbsp;주문 완료</div>
        <div>

<form name="form" method="post" action="order_paying.php" onsubmit="return paypay()">
           <ul id ="ulTable">
               <li>   <!-- 첫 줄 li는 게시판 분류 -->
                   <ul>
                       <li>전체</li>
  ...
  <?php
        ...
        $price = 0; // 총 상품가격에 세팅할 값을 저장하기 위한 변수. 반복문에서 상품 수량별 가격을 꺼내면서 더할 것임

        for( $ii = 0; $ii < $n ; $ii++  ) { // array에 담긴 값을 차례로 반복문으로 꺼내기 위해 $ii = 0으로 초기값을 세팅함

            $querym = "SELECT * from product where no = $product_id[$ii]"; // 상품id에 맞는 정보를 가져오기 위해 반복문 돌때마다 쿼리를 새로 실행함
            $resultm = mysqli_query($connnm, $querym);
            $row_m = mysqli_fetch_array($resultm);            ?>

              <li>  <!-- 게시물이 출력될 영역 -->
                      <ul id="tt">
                        <li><?php echo $iii;?></li>  <!-- 1번, 2번, 3번... (상품갯수마다 번호를 붙임) -->
                        <li style="text-align: right;"><img src="<?php echo $row_m[image_main];?>" valign="middle" width="50px" height="50px" style="margin-top: 5px; margin-left: 10px; margin-right: 10px;"/></li>
                        <li style="text-align: left;"><?php echo $row_m[name];?></li>
                        <li><?php echo number_format($row_m[price]);?><span class="pricereal_hid" hidden="hidden"><?php echo $row_m[price];?></span></li>
                        ...
                        <li>
                            ...
                            <input name="product_amount[]" type="number" class="quantity" value="<?php echo $product_amount[$ii];?>" style="width: 35px;" readonly />

                            <input name="product_price[]" type="hidden" value="<?php echo $product_price[$ii];?>" />
                            <input name="product_id[]" type="hidden" value="<?php echo $product_id[$ii];?>" />
                            <input name="basket_no[]" type="hidden" value="<?php echo $basket_no[$ii];?>" />

                            <input name="item_ea" type="hidden" value="<?php echo $n;?>" />
                          ...
                          <li class="pricereal" id="price"><?php echo number_format($product_price[$ii]);?></li>
                    </ul>
              </li>
<?php
        ...
        $price += $product_price[$ii]; // 총 상품가격에 세팅할 값을 계산
        $iii++;
      }
      ?>
      ...
      <div style="width: 700px; margin: 0 auto;">
         <div id="register_form" class="contents">
           ...
           <div id="register">
              ...
              <label>수령인명</label> <input type="text" name="receiver" id="receiver" style="margin-bottom: 5px;" value="<?php echo $row_e[name]; ?>" readonly/><br>
              <label>휴대폰 번호</label> <input type="tel" name="inputphone1" id="phonenum" style="margin-bottom: 5px;" value="<?php echo $row_e[phone_number]; ?>" readonly/><br>
            ...
            <!-- placeholder => 텍스트필드 내에 있는 짧은 도움말 (흐린색) -->
              <input type="text" style="position: relative; right: 80px; margin-bottom: 5px;" readonly
                placeholder="우편번호" name="inputaddress_number" id="address_num" value="<?php echo $row_e[address_number]; ?>">
            ...
            <input type="text" style="margin-left: 120px; width: 350px; margin-bottom: 5px;" readonly
              placeholder="주소" name="inputaddress_main" id="address_main" value="<?php echo $row_e[address_main]; ?>"><br>
            <input type="text" style="margin-left: 120px; width: 170px;" readonly
            placeholder="상세주소" name="inputaddress_detail" id="address_detail" value="<?php echo $row_e[address_detail]; ?>">
            <input type="text" style="width: 158px;" placeholder="참고항목" name="inputaddress_ref" id="address_ref" value="<?php echo $row_e[address_ref]; ?>" readonly>
         ...
         <div>
           <ul id="payment">
               ...
               <li>   <!-- 첫 줄 li는 게시판 분류 -->
                   <ul>
                       <li class="productprice"><?php echo number_format($price); ?></li>
                       <li style="width: 70px; height: 50px;"> </li>
                       <li class="deliveryprice">2,500</li>
                       <li style="width: 70px; height: 50px;"> </li>
                       <li class="totalprice"><?php echo number_format($price+2500); ?></li>
                   </ul>
               </li>
          </ul>
