...
<html lang="en" dir="ltr">
  <head>
    ...
    #ulTable > li > ul > li:first-child +li                 {width:15%;} /* 구분 열 크기*/
    #ulTable > li > ul > li:first-child +li+li              {width:40%; text-align: left;} /* 제목 열 크기*/
    #ulTable > li > ul > li:first-child +li+li+li           {width:10%;} /* 작성자 열 크기*/
    #ulTable > li > ul > li:first-child +li+li+li+li        {width:10%;} /* 등록일자 열 크기*/
  ...
  <body>
    ...
    <ul id ="ulTable" style="margin-top: 40px;">
        ...
        $start = ($pageNum-1) * $list; // 페이징의 페이지에 따라 DB에서 데이터를 꺼내올 시작점
                     ...
                     $sql = "SELECT * FROM qna order by no DESC LIMIT $start, $list";// qna 안에 있는 모든 항목들중 no 번호에 따라 내림차순으로 5개를 출력하라
  ...
  while($row = mysqli_fetch_array($result)) { ?>
        ...
        <li>  <!-- 게시물이 출력될 영역 -->
            <ul id="tt">
                ...
                <li><a href="./readqna.php?no=<?php echo $row['no'];?>" style="text-decoration: none; color: black;"><?php echo $row['title'];?></a></li>
    ...
    <div id="divPaging">
      <?php
                  ...
                  $sqlzx = "SELECT * FROM qna"; // mysqli_query([연결객체], [쿼리])는 mysqli_connect를 통해 연결된 객체를 이용해 mysql 쿼리를 실행시키는 함수
